...
| 02 | Stacking layers — ReLU, pooling, and hierarchy | planned |
...
The code, prose and documentation are in English. The **rendered output** —
every label baked into a GIF or figure — is themed and localised separately:

```python
style.use_project_style(theme="dark")   # "light" | "dark"
text.set_language("pt-BR")              # "en" | "pt-BR"
```
...
accept them but transcode to video anyway. Pass `mp4=True` to `animate()` and
an H.264 file is written beside the GIF — usually *smaller*, and actually
postable. Keep the GIF for GitHub, docs and messaging.
...
- **Feature-map grids share one scale across channels.** Per-panel scaling
  ...
  single most misleading thing a contact sheet can do.
